...
class Model_orders extends CI_Model
{
	...
	public function getOrdersItemData($order_id = null)
	{
		...
		$sql = "SELECT order_items.*, products.name FROM order_items left join products on order_items.product_id =products.id WHERE order_id = ?";
		$query = $this->db->query($sql, array($order_id));
		return $query->result_array();
	}


	public function getExists($table_id) {
		$this->db->select('id');
		$this->db->where(array('table_id' => $table_id, 'paid_status' => '2') );
	...
	public function createHead()
	{
		...
		$vat_charge = $company['vat_charge_value']; 
	...
	public function get_gross($orderid) {
		$myquery = "SELECT sum(amount) as gross_amount from order_items where order_id = ? " ; 
		$exec  = $this->db->query($myquery, $orderid);
		$val = $exec->row();
		return $val->gross_amount ;
	}
	
	public function get_vat($orderid) {
		$this->db->select('vat_charge_rate');
		$this->db->where('id', $orderid) ;
		$exec = $this->db->get('orders');
		$val = $exec->row();
		return $val->vat_charge_rate;
		
	}
	
	public function updateHead($orderID) {
		$gross = $this->get_gross($orderID);
		$vat_rate = $this->get_vat($orderID);
		
		$vat_value = $gross * ($vat_rate/100) ; 
		
		$net_amount = $gross + $vat_value ;
		
		$toUpdate = array (
			'gross_amount' => $gross, 
			'vat_charge_amount' => $vat_value ,
			'net_amount' => $net_amount
		);
		
		
		$this->db->set($toUpdate)
			  ->where('id', $orderID) ; 
	...
	public function createDetail($data) {
		$exec = $this->db->insert('order_items', $data) ;
